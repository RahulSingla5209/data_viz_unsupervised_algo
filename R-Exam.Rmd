...
Only term frequency vector is used for making the predictions. 
...
Going from 793 features to just 200 features with PCA.

Explained proportion with 200 components is 60% which good enough dimensional reduction and without much loss in prediction accuracy.
...
Baseline accuracy = 2% (50 classes)
...
Anything with support >0.1 is clearly an outlier. So, we will remove those rules.

Looking at the graph above, Lift threshold should be 2 to make sure we evaluate only strongh associations
...
Bidirectional rules with high light

1. Ham -> whitebread & Whitebread -> Ham
2. Hygnie Articles -> Napkins & Napkins -> Hygnie Articles
